    ...
    <div className="flex gap-6">
      {props.products.map((e) =>
        <div key={e.id} className={`${props.cls} h-[696px] bg-white rounded-lg flex flex-col items-center gap-y-8 py-4 px-2`}>
          ...
          <p className="text-lg text-center text-grey-500">Tỷ lệ lợi nhuận cam kết: {e.profitRateCommitment}%</p>
          <p className="text-lg text-center text-grey-500">Tài khoản miễn phí cho website: {e.acountFeeForWebsite}</p>
          <p className="ml-2 text-sx text-black text-center">{e.description}</p>
          <Button variant="primary" className="w-[120px] mb-4" onClick={() => handleClick(e.id)}>Mua</Button>
        </div>
      )}
    </div>
  );
